...
class Trainer(torch.nn.Module):
    ...
    def forward(self, target, it, if_normal, if_pretrain, scene_and_vertices ):
        ...
        if if_pretrain:        
            ...
            return self.geometry.tick(self.glctx, target, self.light, self.material, it , if_normal, self.guidance, self.FLAGS.mode, self.if_flip_the_normal, self.if_use_bump)
...
def appeareance_pass(opt):
    ...
    FLAGS.save_interval = 1000
    FLAGS.video_interval = 10
    FLAGS.min_roughness = 0.08
    FLAGS.custom_mip = False
    FLAGS.random_textures = False
    FLAGS.train_background = 'black'
    FLAGS.out_dir = None
    FLAGS.ref_mesh = None
    ...
    FLAGS.local_rank = 0
    FLAGS.seed = 42
    FLAGS.add_directional_text = False
    FLAGS.mode = 'geometry_modeling'
    FLAGS.text = ""
    FLAGS.sdf_init_shape = 'ellipsoid'
    FLAGS.camera_random_jitter = 0.4
    FLAGS.fovy_range = [25.71, 45.00]
    FLAGS.elevation_range = [-10, 45]
    FLAGS.guidance_weight = 100
    FLAGS.sds_weight_strategy = 0
    FLAGS.translation_y = 0
    FLAGS.translation_z = 0
    FLAGS.coarse_iter = 1000
    FLAGS.early_time_step_range = [0.02, 0.5]
    FLAGS.late_time_step_range = [0.02, 0.5]
    FLAGS.sdf_init_shape_rotate_x = 0
    FLAGS.if_flip_the_normal = False
    FLAGS.front_threshold = 45
    FLAGS.if_use_bump = True
    FLAGS.uv_padding_block = 4
    FLAGS.negative_text = ""
    ...
    if FLAGS.out_dir is None:
        FLAGS.out_dir = 'logs/cube_%d' % (FLAGS.train_res)
    else:
        FLAGS.out_dir = 'logs/' + FLAGS.out_dir
